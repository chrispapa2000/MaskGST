...
class StoryImageDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if os.path.exists("../pikles/my_description_dict.pkl"):
            with open("../pikles/my_description_dict.pkl",'rb') as f:
        ...
        if text_encoder != None:
            with open(f"../pikles/preloaded_captions_{text_encoder}_{mode}", 'rb') as f:
    ...
    def __getitem__(self, item):
        ...
        if self.return_token_indices:
            with open(os.path.join('../pikles','image_tokens',f"{src_img_path_id}.pkl"), 'rb') as f:
        ...
        if self.return_captions:
            ...
            if self.return_token_indices:
                ...
                return torch.stack(images), torch.stack(text_embeddings), labels, masks, image_captions
        
        if self.return_token_indices:
            return torch.stack(images), torch.stack(text_embeddings), token_inds, labels, masks
        else:
            return torch.stack(images), torch.stack(text_embeddings), labels, masks
